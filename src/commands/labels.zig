...
pub fn run(ctx: Context) !u8 {
    ...
    if (has_next and !ctx.json_output) {
        ...
        try stderr.print("labels list: more labels available; pagination not implemented (endCursor {s})\n", .{cursor_value});
    }
...
pub fn usage(writer: anytype) !void {
    try writer.print(
        \\Usage: linear labels list [--team ID|KEY] [--limit N] [--fields LIST] [--plain] [--no-truncate] [--quiet] [--data-only] [--help]
        \\Flags:
        \\  --team ID|KEY    Filter by team id or key
        \\  --limit N        Number of labels to fetch (default: 50)
