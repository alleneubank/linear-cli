...
pub fn run(ctx: Context) !u8 {
    ...
    if (opts.max_items) |max_value| {
        if (max_value == 0) {
            try stderr.print("states list: invalid --max-items value\n", .{});
            return 1;
        }
    }
    ...
    while (true) {
        ...
        if (take_count == 0 or allowed_count == 0) {
            if (has_next) {
                try stderr.print("states list: received empty page; stopping pagination\n", .{});
            }
            break;
        }
        ...
        if (progress.end_cursor == null) {
            try stderr.print("states list: missing endCursor for additional page\n", .{});
            break;
        }
    ...
    if (opts.quiet) {
        ...
        var states_obj = std.json.Value{ .object = std.json.ObjectMap.empty };
        try states_obj.object.put(var_alloc, "nodes", nodes_value);
        try states_obj.object.put(var_alloc, "pageInfo", try pageInfoValue(var_alloc, progress));

        var root_obj = std.json.Value{ .object = std.json.ObjectMap.empty };
        try root_obj.object.put(var_alloc, "workflowStates", states_obj);
        try printer.printJson(root_obj, stdout_iface, true);
    } else {
        try printer.printStateTable(ctx.allocator, stdout_iface, rows.items, selected_fields, table_opts);
    }

    try common.printPageSummary(stderr, "states list", progress, ctx.json_output);
    return 0;
}
...
pub fn usage(writer: anytype) !void {
    try writer.print(
        \\Usage: linear states list [--team ID|KEY] [--limit N] [--max-items N] [--cursor CURSOR] [--pages N|--all] [--fields LIST] [--plain] [--no-truncate] [--quiet] [--data-only] [--help]
        \\Flags:
        ...
        \\  --max-items N    Stop after emitting N states (may truncate within a page)
        ...
        \\  linear states list --all --fields id,name,team --data-only
