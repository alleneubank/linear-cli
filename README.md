...
Run `linear auth status` to see which one is actually supplying the key. It never prints the key itself, so it is safe in a terminal recording or a bug report.
...
- `auth status [--json]` — report which backend supplied the key and whether it validates, without ever printing the key. Exits non-zero when no usable key was found.
...
- `config show|set|unset` — view or update CLI defaults (team/output/state filter) without editing JSON. `credential_helper` is shown and can be unset here, but is only *set* by `auth migrate --to helper`, which verifies it first.
- `me` — show current user.
- `teams list [--fields id,key,name] [--plain] [--no-truncate]` — list teams with optional column and formatting controls.
- `labels list [--team ID|KEY] [--limit N] [--fields id,name,color,description,team] [--plain] [--no-truncate] [--quiet] [--data-only]` — list issue labels; supplies the raw ids required by `issues list --label` and `issue create --labels`.
- `users list [--limit N] [--include-inactive] [--fields id,name,display_name,email,active] [--plain] [--no-truncate] [--quiet] [--data-only]` — list workspace members (active only unless `--include-inactive`); supplies the raw ids required by `issues list --assignee` and `issue create --assignee`.
- `states list [--team ID|KEY] [--limit N] [--fields id,name,type,position,team] [--plain] [--no-truncate] [--quiet] [--data-only]` — list workflow states; supplies the raw ids required by `issues list --state-id` and `issue create --state`. Workflow states are per-team and several teams commonly share state names, so the `Team` column is on by default to keep an unfiltered listing unambiguous.
...
- `milestone list --project ID|NAME [--limit N] [--fields id,name,target_date,sort_order,description,project] [--plain] [--no-truncate] [--quiet] [--data-only]` — list a project's milestones; supplies the raw ids required by `issues list --milestone`. `--project` takes a project uuid, slug, or exact name; an ambiguous name is an error rather than a silent pick.
