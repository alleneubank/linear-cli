...
- Pagination: `issues list` fetches 25 items per page, the other list commands 50; `--cursor`, `--pages`, and `--all` drive additional page fetches, `--max-items` caps the total (it may truncate mid-page), and stderr reports fetched counts plus the `--cursor` value to resume from. The summary is suppressed under `--json`. `search` is the one command that cannot paginate.
