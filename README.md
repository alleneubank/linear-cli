...
- `search <query> [--team ID|KEY] [--fields title,description,comments] [--state-type TYPES] [--assignee USER_ID|me] [--limit N] [--max-items N] [--cursor CURSOR] [--pages N|--all] [--case-sensitive] [--plain] [--no-truncate] [--quiet] [--data-only]` — server-side search over titles/descriptions/comments or identifiers (identifier filter matches issue numbers). Paginates with cursor support plus page summaries, like the other list commands. Note `--fields` here selects which fields to **search**, not which to print; the printed columns are fixed.
